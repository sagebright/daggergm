...
async function verify() {
  // Get all DB records
  const { data: dbRecords } = await s.from('daggerheart_abilities').select('name').order('name')

  // Get all source files
  const files = await readdir('/Users/jmfk/Repos/daggergm_backup/daggerheart-srd/abilities')
